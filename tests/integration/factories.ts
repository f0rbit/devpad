...
function createTask(overrides: UpsertTaskOverrides): Task {
	const id = getNextId();
	return {
		title: `Test Task ${id}`,
		...
		// v2.4 graph columns — mirror the schema defaults (packages/schema/src/database/schema.ts)
		parent_id: null,
		rank: "",
		rev: 0,
		kind: "task",
...
function createTag(owner_id: string, overrides: Partial<UpsertTag> = {}): Tag {
	const id = getNextId();
	return {
		id: id,
		owner_id,
...
function createTestTags(owner_id: string, count: number = 3): Tag[] {
