...
import { E2E_CANVAS_STAGING_PROJECT_ID, seed_canvas_staging_fixtures } from "./fixtures/canvas-staging";
...
export async function seed(): Promise<void> {
	...
	try {
		...
		await seed_canvas_staging_fixtures(db);
		console.log(
			`[e2e seed] seeded canvas-home staging-shape fixtures into ${db_file} (project=${E2E_CANVAS_STAGING_PROJECT_ID})`,
		);
	} finally {
		sqlite.close();
	}
