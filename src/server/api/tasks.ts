		...
		const fetchedTask =
			(await prisma?.task.update({
				...
				data: {
					...
					owner_id: session?.user?.id,
					modules: {
						upsert: task.modules.map((module) => ({
							where: { task_id_type: { task_id: task.id, type: module.type } },
							create: { type: module.type, data: module.data as Prisma.InputJsonObject },
							update: { data: module.data as Prisma.InputJsonObject }
						})) as Prisma.Enumerable<Prisma.TaskModuleUpsertWithWhereUniqueWithoutTaskInput>
					},
					tags: {
