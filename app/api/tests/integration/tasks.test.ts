...
import type { TaskUnion } from '../../src/types/tasks';

describe('tasks API client integration', () => {
  const createdProjects: Project[] = [];
  const createdTasks: TaskUnion[] = [];
  const testOwnerId = TEST_USER_ID;
  ...
  afterAll(async () => {
    ...
    for (const task of createdTasks) {
      try {
        await test_client.tasks.delete(task.task.id);
      } catch (error) {
    ...
    for (const project of createdProjects) {
      try {
        await test_client.projects.delete(project.id);
      } catch (error) {
  ...
  test('should create a new task', async () => {
    const taskData = TestDataFactory.createRealisticTask(testOwnerId);
    ...
    expect(createdTask.task.owner_id).toBe(testOwnerId);
    expect(Array.isArray(createdTask.tags)).toBe(true);
  });

  test('should create a task with tags', async () => {
    const taskData = TestDataFactory.createRealisticTask(testOwnerId);
    const tags = TestDataFactory.createTestTags(testOwnerId, 2);
  ...
  test('should create task within a project', async () => {
    ...
    const taskData = TestDataFactory.createRealisticTask(testOwnerId, createdProject.id);
    const createdTask = await test_client.tasks.create(taskData);
    createdTasks.push(createdTask);
  ...
  test('should update an existing task', async () => {
    // First create a task
    const taskData = TestDataFactory.createTask(testOwnerId);
    const createdTask = await test_client.tasks.create(taskData);
    createdTasks.push(createdTask);
  ...
  test('should get tasks by project', async () => {
    ...
    const task1 = await test_client.tasks.create(
      TestDataFactory.createTask(testOwnerId, { project_id: createdProject.id })
    );
  ...
  test('should complete full project and task workflow', async () => {
    ...
    const task1Data = TestDataFactory.createRealisticTask(testOwnerId, project.id, {
      title: 'Setup project structure',
      priority: 'HIGH'
    });
    const task2Data = TestDataFactory.createRealisticTask(testOwnerId, project.id, {
      title: 'Write tests',
      priority: 'MEDIUM'
    });
    ...
    const updatedProject = await test_client.projects.update(project.id, {
      status: 'PAUSED'
    });
