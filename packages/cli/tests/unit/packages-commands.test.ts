...
import { action_packages_create, action_packages_delete, action_packages_get, action_packages_list, action_packages_update } from "../../src/commands/pipelines";
...
const err = (message: string, status = 500, code = "unknown"): ApiErr => ({ ok: false, error: { message, status, code } });
...
const build_fake_client = (overrides: Partial<FakeClient["pipelines"]["packages"]> = {}, opts: { projects?: Array<{ id: string; project_id: string }>; session_user_id?: string | null; session_ok?: boolean } = {}): FakeClient => {
	...
	return {
		...
		pipelines: {
			packages: {
				list: async filter => {
					...
					return (overrides.list ? overrides.list(filter) : ok([{ id: "pipeline-package_a", name: "a", project_id: null, repo_url: null }])) as Promise<ApiResult<unknown[]>>;
				},
				get: async id => {
					calls.push({ kind: "get", args: [id] });
					return (overrides.get ? overrides.get(id) : ok({ id, name: "a", project_id: null })) as Promise<ApiResult<unknown>>;
				},
				create: async input => {
					calls.push({ kind: "create", args: [input] });
					return (overrides.create ? overrides.create(input) : ok({ ...((input as Record<string, unknown>) ?? {}), id: (input as { id: string }).id })) as Promise<ApiResult<unknown>>;
				},
				update: async (id, patch) => {
					calls.push({ kind: "update", args: [id, patch] });
					return (overrides.update ? overrides.update(id, patch) : ok({ id, ...((patch as Record<string, unknown>) ?? {}) })) as Promise<ApiResult<unknown>>;
				},
				delete: async id => {
					calls.push({ kind: "delete", args: [id] });
					return (overrides.delete ? overrides.delete(id) : ok({ deleted: true })) as Promise<ApiResult<{ deleted: true }>>;
				},
			},
		},
		projects: {
		...
		auth: {
			session: async () => {
				...
				return ok({ authenticated: true, user: opts.session_user_id === null ? null : { id: opts.session_user_id ?? "user_session" }, session: null });
			},
		},
	};
};
...
describe("action_packages_create", () => {
	...
	test("--project slug is resolved to project_id", async () => {
		const client = build_fake_client({}, { session_user_id: "user_session", projects: [{ id: "internal_a", project_id: "forbit-dev" }] });
		await action_packages_create(() => client as never)("forbit-astro", { project: "forbit-dev", repoUrl: "https://github.com/f0rbit/forbit-astro" });
		const create_calls = client.calls.filter(c => c.kind === "create");
...
describe("action_packages_update", () => {
	...
	test("parses --script-name-overrides JSON", async () => {
		const client = build_fake_client();
		await action_packages_update(() => client as never)("pipeline-package_x", { scriptNameOverrides: '{"staging":"x-stg"}' });
		const update_calls = client.calls.filter(c => c.kind === "update");
