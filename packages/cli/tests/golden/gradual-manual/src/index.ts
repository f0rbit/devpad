/**
 * gradual-manual — pipeline-managed Cloudflare Worker scaffold.
 *
...
export class GradualManualWorker extends WorkerEntrypoint<Env> {
	override async fetch(request: Request): Promise<Response> {
		const url = new URL(request.url);
		if (url.pathname === "/health") {
			return Response.json({ status: "ok", service: "gradual-manual" });
		}
		if (url.pathname === "/version") {
			return Response.json({ name: "gradual-manual", version: "0.0.1" });
		}
		return new Response("Not Found", { status: 404 });
	}
}

export default GradualManualWorker;
