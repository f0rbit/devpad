/**
 * Alchemy infrastructure declaration for directory-bundle.
 *
 ...
 *
 * Platform services (vault, pulse) are singletons — `vault` and
 * `pulse-api` are the only Workers, regardless of which stage of this
 * package is calling. Stage scoping happens at the `caller.environment`
 * RPC arg (vault grants) and on pulse event tags, not by routing to a
 * different upstream Worker.
 *
...
const app = await alchemy("directory-bundle");

const stage = app.stage;
const environment = stage === "staging" ? "staging" : "production";
const worker_name = stage === "staging" ? "directory-bundle-staging" : "directory-bundle";
...
if (pulse_project_id !== undefined && pulse_ingest_key_value !== undefined) {
	...
	const secrets_store = await SecretsStore("directory-bundle-secrets", {
...
export const worker = await Worker("directory-bundle", {
	name: worker_name,
