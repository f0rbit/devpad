/**
 * Pipeline declaration for directory-bundle.
 *
...
export default extendTemplate({
		...
		gates: {
			"staging→onebox": auto({ afterBake: true }),
			"onebox→wave1": auto({ afterBake: true }),
			"wave1→wave2": auto({ afterBake: true }),
			"wave2→full": auto({ afterBake: true }),
		},
});
