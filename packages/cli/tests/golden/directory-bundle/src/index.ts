/**
 * directory-bundle — pipeline-managed Cloudflare Worker scaffold.
 *
...
export class DirectoryBundleWorker extends WorkerEntrypoint<Env> {
	override async fetch(request: Request): Promise<Response> {
		const url = new URL(request.url);
		if (url.pathname === "/health") {
			return Response.json({ status: "ok", service: "directory-bundle" });
		}
		if (url.pathname === "/version") {
			return Response.json({ name: "directory-bundle", version: "0.0.1" });
		}
		return new Response("Not Found", { status: 404 });
	}
}

export default DirectoryBundleWorker;
