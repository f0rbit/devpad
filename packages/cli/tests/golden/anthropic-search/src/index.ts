/**
 * anthropic-search — pipeline-managed Cloudflare Worker scaffold.
 *
 ...
 */
export const emit_pulse = async (env: Env, event_name: string, properties: Record<string, unknown>): Promise<void> => {
...
export class AnthropicSearchWorker extends WorkerEntrypoint<Env> {
	override async fetch(request: Request): Promise<Response> {
		const url = new URL(request.url);
		if (url.pathname === "/health") {
			return Response.json({ status: "ok", service: "anthropic-search" });
		}
		if (url.pathname === "/version") {
			return Response.json({ name: "anthropic-search", version: "0.0.1" });
		}
		return new Response("Not Found", { status: 404 });
	}
}

export default AnthropicSearchWorker;
