...
import type { AnthropicError, AnthropicProvider, CreateMessagesInput, CreateMessagesOutput } from "./anthropic-provider.ts";
...
export class InMemoryAnthropicProvider implements AnthropicProvider {
	...
	readonly messages = {
		create: async (input: CreateMessagesInput): Promise<Result<CreateMessagesOutput, AnthropicError>> => {
			...
			const input_tokens = input.messages.reduce((acc, m) => acc + count_tokens(m.content), 0) + count_tokens(input.system ?? "");
