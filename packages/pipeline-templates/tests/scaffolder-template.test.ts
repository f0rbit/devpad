...
const render_config = (package_name: string, compatibility_date: string): unknown => {
	...
	const rendered = template_content.replace(/{{package_name}}/g, package_name).replace(/{{compatibility_date}}/g, compatibility_date);
...
describe("scaffolder wrangler.jsonc template", () => {
	...
	test("rendered template has required bindings", () => {
		const config = render_config("test-package", "2026-05-17") as { services: Array<{ binding: string; service: string; entrypoint?: string }> };
	...
	test("rendered template has staging/production env blocks", () => {
		const config = render_config("test-package", "2026-05-17") as { env: { staging: { name: string }; production: { name: string } } };
