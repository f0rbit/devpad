...
import { defaultAtomic, defaultGradual, expand_rollout, extendTemplate } from "../src/index.ts";
...
describe("expand_rollout", () => {
	...
	test("stage[0] of any rollout is always 'staging' (orchestrator entry point)", () => {
		...
		expect(expand_rollout({ type: "gradual", stages: [{ name: "x", traffic: 100, bake: { ms: 0 } }] })[0].name).toBe("staging");
	});
});
