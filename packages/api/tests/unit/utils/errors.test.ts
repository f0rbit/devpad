...
import { DevpadApiError, NetworkError, AuthenticationError, ValidationError } from '../../../src/utils/errors';

describe('DevpadApiError', () => {
  test('should create basic error with message', () => {
    const error = new DevpadApiError('Something went wrong');
    
    expect(error).toBeInstanceOf(Error);
    expect(error).toBeInstanceOf(DevpadApiError);
    expect(error.name).toBe('DevpadApiError');
  ...
  test('should create error with code and statusCode', () => {
    const error = new DevpadApiError('Bad request', {
      code: 'INVALID_INPUT',
      statusCode: 400
    });
  ...
  test('should handle partial options', () => {
    const errorWithCode = new DevpadApiError('Error with code', {
      code: 'TEST_CODE'
    });
    expect(errorWithCode.code).toBe('TEST_CODE');
    expect(errorWithCode.statusCode).toBeUndefined();

    const errorWithStatus = new DevpadApiError('Error with status', {
      statusCode: 500
    });
  ...
  test('should create error from Response object', () => {
    ...
    const error = DevpadApiError.fromResponse(mockResponse);
...
describe('AuthenticationError', () => {
  ...
  test('should inherit from DevpadApiError', () => {
    const error = new AuthenticationError();
    
    expect(error instanceof DevpadApiError).toBe(true);
...
describe('Error stack traces and serialization', () => {
  test('should preserve stack traces', () => {
    const error = new DevpadApiError('Test error');
    
    expect(error.stack).toBeDefined();
    expect(typeof error.stack).toBe('string');
    expect(error.stack).toContain('DevpadApiError');
  });
  ...
  test('should be serializable to JSON', () => {
    ...
    expect(parsed.name).toBe('DevpadApiError');
  ...
  test('should handle toString() correctly', () => {
    ...
    expect(errorString).toContain('DevpadApiError');
    expect(errorString).toContain('Test error');
  });
});

describe('Error inheritance chain', () => {
  test('should maintain proper prototype chain for DevpadApiError', () => {
    const error = new DevpadApiError('Test');
    
    expect(Object.getPrototypeOf(error)).toBe(DevpadApiError.prototype);
    expect(Object.getPrototypeOf(DevpadApiError.prototype)).toBe(Error.prototype);
  });
  ...
  test('should maintain proper prototype chain for specialized errors', () => {
    ...
    expect(Object.getPrototypeOf(AuthenticationError.prototype)).toBe(DevpadApiError.prototype);
    expect(Object.getPrototypeOf(NetworkError.prototype)).toBe(DevpadApiError.prototype);
    expect(Object.getPrototypeOf(ValidationError.prototype)).toBe(DevpadApiError.prototype);
  });

  test('should work correctly with instanceof checks', () => {
    const baseError = new DevpadApiError('Base error');
    ...
    expect(baseError instanceof DevpadApiError).toBe(true);
    expect(baseError instanceof AuthenticationError).toBe(false);

    // Authentication error
    expect(authError instanceof Error).toBe(true);
    expect(authError instanceof DevpadApiError).toBe(true);
    ...
    expect(networkError instanceof DevpadApiError).toBe(true);
    ...
    expect(validationError instanceof DevpadApiError).toBe(true);
