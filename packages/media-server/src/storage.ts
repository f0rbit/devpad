...
const listCommits = async (backend: Backend, accountId: string): Promise<RepoStoreInfo[]> => {
	const metaResult = createTypedStore(backend, STORE_PATTERNS.githubMeta(accountId), GitHubMetaStoreSchema);
	...
	return latestResult.value.data.repositories.map((repo: { owner: string; name: string }) => ({
		owner: repo.owner,
		repo: repo.name,
		storeId: STORE_PATTERNS.githubCommits(accountId, repo.owner, repo.name),
	}));
};

const listPRs = async (backend: Backend, accountId: string): Promise<RepoStoreInfo[]> => {
	const metaResult = createTypedStore(backend, STORE_PATTERNS.githubMeta(accountId), GitHubMetaStoreSchema);
	if (!metaResult.ok) return [];
	...
	return latestResult.value.data.repositories.map((repo: { owner: string; name: string }) => ({
		owner: repo.owner,
		repo: repo.name,
		storeId: STORE_PATTERNS.githubPRs(accountId, repo.owner, repo.name),
	}));
};
...
export const store = {
	...
	id: {
		raw: STORE_PATTERNS.raw,
		timeline: STORE_PATTERNS.timeline,
		github: {
			meta: STORE_PATTERNS.githubMeta,
			commits: STORE_PATTERNS.githubCommits,
			prs: STORE_PATTERNS.githubPRs,
		},
		reddit: {
			meta: STORE_PATTERNS.redditMeta,
			posts: STORE_PATTERNS.redditPosts,
			comments: STORE_PATTERNS.redditComments,
		},
		twitter: {
			meta: STORE_PATTERNS.twitterMeta,
			tweets: STORE_PATTERNS.twitterTweets,
		},
	},
};
