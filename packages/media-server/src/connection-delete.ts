...
const resolveStoreFromId = (backend: Backend, storeId: string) => {
	...
	switch (storeType) {
		case "github_meta":
			return store.github.meta(backend, parsed.value.accountId);
		case "github_commits":
			return store.github.commits(backend, parsed.value.accountId, parsed.value.owner, parsed.value.repo);
		case "github_prs":
			return store.github.prs(backend, parsed.value.accountId, parsed.value.owner, parsed.value.repo);
		case "reddit_meta":
			return store.reddit.meta(backend, parsed.value.accountId);
		case "reddit_posts":
			return store.reddit.posts(backend, parsed.value.accountId);
		case "reddit_comments":
			return store.reddit.comments(backend, parsed.value.accountId);
		case "twitter_meta":
			return store.twitter.meta(backend, parsed.value.accountId);
		case "twitter_tweets":
			return store.twitter.tweets(backend, parsed.value.accountId);
		case "raw":
...
const deleteGitHubStores = async (backend: Backend, accountId: string): Promise<string[]> => {
	...
	const commitStores = await store.github.list.commits(backend, accountId);
