...
const aggregateRefreshResults = (attempts: RefreshAttempt[]): { succeeded: number; failed: number; errors: string[] } =>
	attempts.reduce(
		(acc, a) => ({
...
const fetchActiveAccountsForUser = async (db: Database, userId: string): Promise<AccountWithUser[]> =>
	db
...
const createConnection = async (ctx: AppContext, uid: UserId, input: ConnectionInput): Promise<Result<{ account_id: string; profile_id: string }, ServiceError>> => {
	...
	const encryptedAccessToken = await secrets.encrypt(input.access_token, ctx.encryptionKey);
	...
	if (input.refresh_token) {
		const refreshResult = await secrets.encrypt(input.refresh_token, ctx.encryptionKey);
		if (!refreshResult.ok) {
...
const getGitHubRepos = async (ctx: AppContext, uid: UserId, accId: AccountId): Promise<Result<{ repos: GitHubRepoInfo[] }, ServiceError>> => {
	...
	const metaStoreResult = store.github.meta(ctx.backend, accId);
...
const getRedditSubreddits = async (ctx: AppContext, uid: UserId, accId: AccountId): Promise<Result<SubredditResult, ServiceError>> => {
	...
	const metaStoreResult = store.reddit.meta(ctx.backend, accId);
	if (!metaStoreResult.ok) {
...
const attemptRedditTokenRefresh = async (ctx: AppContext, account: RefreshableAccount): Promise<Result<string, RefreshError>> => {
	...
	const refreshTokenResult = await secrets.decrypt(account.refresh_token_encrypted, ctx.encryptionKey);
	...
	const encryptResult = await secrets.encrypt(newAccessToken, ctx.encryptionKey);
	if (!encryptResult.ok) {
...
export const connection = {
	...
	delete: deleteConnectionWithTimelineRegen,
	status: updateConnectionStatus,
	refresh: (ctx: AppContext, accountIdStr: string, uid: string) => refreshSingleAccount(ctx, accountIdStr, uid),
	refreshAll: (ctx: AppContext, uid: string) => refreshAllAccounts(ctx, uid),
	settings: {
		get: getConnectionSettings,
		update: updateConnectionSettings,
	},
	repos: getGitHubRepos,
	subreddits: getRedditSubreddits,
	active: fetchActiveAccountsForUser,
	activeAll: fetchAllActiveAccounts,
	byId: fetchAccountByIdWithStatus,
	categorize: categorizeAccountsByPlatform,
	strategy: determineRefreshStrategy,
	aggregate: aggregateRefreshResults,
	shouldRegen: shouldRegenerateTimeline,
};
