...
export type OAuthError = { kind: "invalid_state" } | { kind: "github_error"; message: string } | { kind: "database_error"; message: string };
...
export async function handleGitHubCallback(db: any, env: OAuthEnv, code: string, state: string, stored_state: string): Promise<Result<OAuthCallbackResult, OAuthError>> {
	...
	if (!session_result.ok) return err({ kind: "database_error", message: `Session creation failed: ${session_result.error.kind}` });
...
async function createOrUpdateUser(db: any, github_user: GitHubUser): Promise<Result<OAuthCallbackResult["user"], OAuthError>> {
	...
	if (existing instanceof Error) return err({ kind: "database_error", message: existing.message });
	...
	if (existing.length > 0) {
		...
		if (updated instanceof Error) return err({ kind: "database_error", message: updated.message });
	...
	if (created instanceof Error) return err({ kind: "database_error", message: created.message });
