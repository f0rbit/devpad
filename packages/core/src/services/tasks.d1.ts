...
import { getTaskTags, upsertTag } from "./tags.d1.js";
...
export async function upsertTask(db: any, data: UpsertTodo, tags: UpsertTag[], owner_id: string): Promise<Result<Task | null, ServiceError>> {
	...
	if (data.owner_id && data.owner_id !== owner_id) {
		return err({ kind: "unauthorized", message: "owner_id mismatch" });
	}

	if (previous && previous.owner_id !== owner_id) {
		return err({ kind: "unauthorized", message: "User does not own this task" });
	}

	if (data.goal_id) {
		const { getGoal } = await import("./goals.d1.js");
		const goal_result = await getGoal(db, data.goal_id);
		if (!goal_result.ok || !goal_result.value) {
			return err({ kind: "validation_error", message: `Goal with id ${data.goal_id} does not exist` });
		}

		const task_project_id = data.project_id ?? previous?.project_id;
		const { getMilestone } = await import("./milestones.d1.js");
		const milestone_result = await getMilestone(db, goal_result.value.milestone_id);
		if (!milestone_result.ok || !milestone_result.value) {
			return err({ kind: "validation_error", message: `Milestone for goal ${data.goal_id} does not exist` });
		}
		if (milestone_result.value.project_id !== task_project_id) {
			return err({ kind: "validation_error", message: `Goal ${data.goal_id} belongs to different project than task` });
		}
	}
	...
	if (!result) return err({ kind: "database_error", message: "Task upsert failed" });
