...
import { getMilestone } from "./milestones.d1.js";
import { doesUserOwnProject } from "./projects.d1.js";
...
export async function getGoal(db: any, goal_id: string): Promise<Result<Goal | null, ServiceError>> {
	...
	if (!record || record.deleted) return err({ kind: "not_found", entity: "goal", id: goal_id });
	return ok(record);
}
...
export async function upsertGoal(db: any, data: UpsertGoal, owner_id: string): Promise<Result<Goal, ServiceError>> {
	...
	if (!milestone_result.value) return err({ kind: "not_found", entity: "milestone", id: data.milestone_id });
	...
	if (!owns_result.value) return err({ kind: "unauthorized", message: "User does not own this project" });

	if (previous && previous.deleted) {
		return err({ kind: "validation_error", message: "Cannot modify deleted goal" });
	}
	...
	if (!result) return err({ kind: "database_error", message: "Goal upsert failed" });
	return ok(result);
}
...
export async function deleteGoal(db: any, goal_id: string, owner_id: string): Promise<Result<void, ServiceError>> {
	...
	if (!goal_result.value) return err({ kind: "not_found", entity: "goal", id: goal_id });

	const milestone_result = await getMilestone(db, goal_result.value.milestone_id);
	if (!milestone_result.ok) return milestone_result;
	if (!milestone_result.value) return err({ kind: "not_found", entity: "milestone", id: goal_result.value.milestone_id });
