...
describe("find_rollback_target", () => {
	test("returns the most recent deployed predecessor", () => {
		const lineage: VersionSetRef[] = [ref("v1", "2026-05-01T00:00:00Z"), ref("v2", "2026-05-05T00:00:00Z"), ref("v3", "2026-05-10T00:00:00Z")];
	...
	test("skips never-deployed entries", () => {
		const lineage: VersionSetRef[] = [ref("v1", "2026-05-01T00:00:00Z", true), ref("v2", "2026-05-05T00:00:00Z", false), ref("v3", "2026-05-10T00:00:00Z")];
		const r = find_rollback_target(lineage, "v3");
		expect(r.ok).toBe(true);
	...
	test("sorts lineage regardless of input order", () => {
		const lineage: VersionSetRef[] = [ref("a", "2026-01-01T00:00:00Z"), ref("c", "2026-03-01T00:00:00Z"), ref("b", "2026-02-01T00:00:00Z")];
