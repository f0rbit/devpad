import type { GitHubRepoCommit, GitHubRepoPR, TimelineItem } from "@devpad/schema/media";
...
export const loadGitHubData = async (backend: Backend, accountId: string): Promise<GitHubTimelineData> => {
	...
	await Promise.all(
		commitStores.map(async ({ owner, repo }: { owner: string; repo: string }) => {
	...
	await Promise.all(
		prStores.map(async ({ owner, repo }: { owner: string; repo: string }) => {
