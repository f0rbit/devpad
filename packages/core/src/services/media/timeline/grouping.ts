import type { CommitGroup, CommitPayload, DateGroup, PullRequestPayload, TimelineItem } from "@devpad/schema/media";
...
export const groupCommits = (items: TimelineItem[]): TimelineEntry[] => {
	...
	const groupedByRepoBranchDate = orphanCommits.reduce<Map<string, CommitItem[]>>((acc, commit) => {
		const dateKey = date.key(commit.timestamp);
		const key = makeGroupKey(commit.payload.repo, commit.payload.branch, dateKey);
