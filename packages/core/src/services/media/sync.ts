...
export const generateTimeline = (userId: string, items: TimelineItem[]) => {
	...
	const entries: TimelineEntry[] = tl.group(items);
	const dateGroups = tl.byDate(entries);
...
const getLatestSnapshot = async (backend: Backend, account: AccountWithUser): Promise<RawSnapshot | null> => {
	if (account.platform === "github") {
		const githubData = await tl.loaders.github(backend, account.id);
	...
	if (account.platform === "reddit") {
		const redditData = await tl.loaders.reddit(backend, account.id);
	...
	if (account.platform === "twitter") {
		const twitterData = await tl.loaders.twitter(backend, account.id);
	...
	const storeResult = store.raw(backend, account.platform, account.id);
	if (!storeResult.ok) {
		return null;
	}
...
export const combineUserTimeline = async (backend: Backend, userId: string, snapshots: RawSnapshot[]): Promise<void> => {
	...
	const generated = generateTimeline(userId, allItems);
	await storeTimeline(backend, userId, generated, snapshots);

	logTimeline.info("Timeline stored", {
		user_id: userId,
		date_groups: generated.groups.length,
...
export const userTimeline = {
	...
	store: storeTimeline,
	combine: combineUserTimeline,
	gather: gatherLatestSnapshots,
	regenerate: regenerateTimelinesForUsers,
	snapshots: {
		group: groupSnapshotsByPlatform,
		normalize: normalizeOtherSnapshots,
	},
	platform: {
		load: loadPlatformItems,
	},
};
